...
export default {
  ...
  methods: {
    ...
    remove() {
      if (this.d_selection) {
        this.$emit("remove", this.d_selection);
      }
    },
    ...
    moveUp() {
      if (this.d_selection) {
        ...
        for (let i = 0; i < this.d_selection.length; i++) {
          ...
          let selectedItemIndex = ObjectUtils.findIndexInList(
            selectedItem,
            value
          );
    ...
    onItemClick(event, item, index) {
      ...
      if (metaSelection) {
        ...
        if (selected && metaKey) {
          this.d_selection = this.d_selection.filter(
            (val, index) => index !== selectedIndex
          );
        } else {
          this.d_selection = metaKey
            ? this.d_selection
              ? [...this.d_selection]
              : []
            : [];
    ...
    findNextItem(item) {
      ...
      if (nextItem)
        return !DomHandler.hasClass(nextItem, "p-orderlist-item")
    ...
    findPrevItem(item) {
      ...
      if (prevItem)
        return !DomHandler.hasClass(prevItem, "p-orderlist-item")
          ? this.findPrevItem(prevItem)
          : prevItem;
      else return null;
    },
    updateListScroll() {
      const listItems = DomHandler.find(
        this.$refs.list.$el,
